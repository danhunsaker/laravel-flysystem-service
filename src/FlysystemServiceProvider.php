...
class FlysystemServiceProvider extends ServiceProvider
{
    ...
    {
        // Replace the built-in filesystems with Eventable counterparts if available
        if (class_exists('\League\Flysystem\EventableFilesystem\EventableFilesystem')) {
            static::$fsClass = \League\Flysystem\EventableFilesystem\EventableFilesystem::class;

            Storage::extend('local', function ($app, $config) {
                $permissions = Arr::get($config, 'permissions', []);

                return new static::$fsClass(new \League\Flysystem\Adapter\Local($config['root'], LOCK_EX, \League\Flysystem\Adapter\Local::DISALLOW_LINKS, $permissions));
            });
            ...
            Storage::extend('ftp', function ($app, $config) {
                ...
                return new static::$fsClass(new \League\Flysystem\Adapter\Ftp($ftpConfig));
            });
            ...
            if (class_exists('\League\Flysystem\AwsS3v3\AwsS3Adapter')) {
                Storage::extend('s3', function ($app, $config) {
                    ...
                    $root = Arr::get($config, 'root', null);

                    return new static::$fsClass(new \League\Flysystem\AwsS3v3\AwsS3Adapter(new \Aws\S3\S3Client($config), $config['bucket'], $root));
                });
            }
            ...
            if (class_exists('\League\Flysystem\Rackspace\RackspaceAdapter')) {
                Storage::extend('rackspace', function ($app, $config) {
                    ...
                    return new static::$fsClass(new \League\Flysystem\Rackspace\RackspaceAdapter($store->getContainer($config['container'])));
                });
            }
        }
        // Otherwise, just set up the extended filesystems with the normal class
        else {
            static::$fsClass = \League\Flysystem\Filesystem::class;
        }

        Storage::extend('null', function ($app, $config) {
            return new static::$fsClass(new \League\Flysystem\Adapter\NullAdapter);
        });
        ...
        if (class_exists('\League\Flysystem\Azure\AzureAdapter')) {
            Storage::extend('azure', function ($app, $config) {
                ...
                return new static::$fsClass(new \League\Flysystem\Azure\AzureAdapter($client, $config['container']));
            });
        }
        ...
        if (class_exists('\League\Flysystem\Copy\CopyAdapter')) {
            Storage::extend('copy', function ($app, $config) {
                ...
                return new static::$fsClass(new \League\Flysystem\Copy\CopyAdapter($client));
            });
        }
        ...
        if (class_exists('\League\Flysystem\Dropbox\DropboxAdapter')) {
            Storage::extend('dropbox', function ($app, $config) {
                ...
                return new static::$fsClass(new \League\Flysystem\Dropbox\DropboxAdapter($client));
            });
        }
        ...
        if (class_exists('\League\Flysystem\GridFS\GridFSAdapter')) {
            Storage::extend('gridfs', function ($app, $config) {
                ...
                return new static::$fsClass(new \League\Flysystem\GridFS\GridFSAdapter($gridFs));
            });
        }

        if (class_exists('\League\Flysystem\Memory\MemoryAdapter')) {
            Storage::extend('memory', function ($app, $config) {
                return new static::$fsClass(new \League\Flysystem\Memory\MemoryAdapter());
            });
        }

        if (class_exists('\League\Flysystem\Phpcr\PhpcrAdapter')) {
            Storage::extend('phpcr', function ($app, $config) {
                ...
                return new static::$fsClass(new \League\Flysystem\Phpcr\PhpcrAdapter($session, $config['root']));
            });
        }

        if (class_exists('\League\Flysystem\Replicate\ReplicateAdapter')) {
            Storage::extend('replicate', function ($app, $config) {
                return new static::$fsClass(new \League\Flysystem\Replicate\ReplicateAdapter(Storage::disk($config['master'])->getAdapter(), Storage::disk($config['replica'])->getAdapter()));
            });
        }

        if (class_exists('\League\Flysystem\Sftp\SftpAdapter')) {
            Storage::extend('sftp', function ($app, $config) {
                return new static::$fsClass(new \League\Flysystem\Sftp\SftpAdapter($config));
            });
        }

        if (class_exists('\League\Flysystem\Vfs\VfsAdapter')) {
            Storage::extend('vfs', function ($app, $config) {
                return new static::$fsClass(new \League\Flysystem\Vfs\VfsAdapter(new VirtualFileSystem\FileSystem()));
            });
        }
        ...
        if (class_exists('\League\Flysystem\WebDAV\WebDAVAdapter')) {
            Storage::extend('webdav', function ($app, $config) {
                ...
                return new static::$fsClass(new \League\Flysystem\WebDAV\WebDAVAdapter(new \Sabre\DAV\Client($config)));
            });
        }

        if (class_exists('\League\Flysystem\ZipArchive\ZipArchiveAdapter')) {
            Storage::extend('zip', function ($app, $config) {
                return new static::$fsClass(new \League\Flysystem\ZipArchive\ZipArchiveAdapter($config['path']));
            });
        }
    }
